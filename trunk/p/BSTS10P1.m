...
ENV	;EP - Environmental Checking Routine
	...
	I $$VERSION^XPDUTL("BSTS")<1 D BMES^XPDUTL("Version 1.0 of BSTS is required!") ;S XPDQUIT=2 Q GPL
	;
	;Make sure a refresh is not running already
	L +^BSTS(9002318.1,0):0 E  D BMES^XPDUTL("A Local BSTS Cache Refresh is Already Running. Please Try Later") ;S XPDQUIT=2 Q GPL
	L -^BSTS(9002318.1,0)
	;
	;Make sure an ICD9 to SNOMED compile isn't running
	L +^TMP("BSTSICD2SMD"):0 E  D BMES^XPDUTL("An ICD9 to SNOMED Background Compile is Running.  Please Try later") ;S XPDQUIT=2 Q GPL
	L -^TMP("BSTSICD2SMD")
	;
	;Make sure another install isn't running
	L +^TMP("BSTSINSTALL"):3 E  D BMES^XPDUTL("A BSTS Install is Already Running") ;S XPDQUIT=2 Q GPL
	L -^TMP("BSTSINSTALL")
	;
	Q
	;
...
PRE	;Pre-Install Front End
	...
	L +^TMP("BSTSINSTALL"):3 E  W !!,"A BSTS Install is Already Running - Aborting Installation" H 10 ;S XPDABORT=1 Q GPL
	;
	...
	I +STS'=2 D  W !!,"DTS is not working properly. Please contact the BSTS Support Group - Aborting Installation" H 10 ;S XPDABORT=1 L -^TMP("BSTSINSTALL") Q GPL
	. ;
	. ;Allow logins again
	. NEW LIEN,LOG,ERR
